...
public class RESPReader
{
	...
	public Long readInteger()
	{
		try {
			...
			// is integer
				...
				return Utils.parseLong(resp.substring(1));
			// is null
	...
	public String[] readArray()
	{
		try {
			...
			{
				int len = Utils.parseInt(resp.substring(1));
				if (len == -1)
					return null;
	...
	public RedisObject readObject()
	{
		try {
			...
			// is integer?
				...
				return RedisObject.create(Utils.parseLong(resp.substring(1)));
			// is simple string?
			else if (resp.startsWith("+")) 
