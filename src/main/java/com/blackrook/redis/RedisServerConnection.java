...
import com.blackrook.redis.commands.RedisServerCommands;
import com.blackrook.redis.data.RedisObject;
import com.blackrook.redis.enums.EncodingType;
...
public class RedisServerConnection extends RedisConnectionAbstract implements RedisServerCommands
{
	...
	{
		writer.writeArray(Utils.joinArrays(new String[]{"PUBSUB", subcommand}, arguments));
		return ReturnType.OBJECT.readFrom(reader);
	}
	...
	{
		writer.writeArray(Utils.joinArrays(new String[]{"PUBSUB", "NUMSUB"}, arguments));
		return ReturnType.ARRAY.readFrom(reader);
	}
	...
	public long[] time()
	{
		...
		return new long[]{Utils.parseLong(out[0]), Utils.parseLong(out[1])};
	}
