...
public class RedisPubSubConnection extends RedisConnectionAbstract implements RedisPubSubCommands
{
	...
	{
		this.listeners = new LinkedList<RedisSubscriptionListener>();
		this.counter = new AtomicLong(0L);
		addListener(listeners);
		(this.subcriptionThread = new SubcriptionThread()).start();
		while (!subcriptionThread.isAlive()) 
	...
	{
		writer.writeArray(Utils.joinArrays(COMMAND_SUBSCRIBE, channels));
	}

	@Override
	public void unsubscribe(String... channels)
	{
		writer.writeArray(Utils.joinArrays(COMMAND_UNSUBSCRIBE, channels));
	}

	@Override
	public void psubscribe(String... patterns)
	{
		writer.writeArray(Utils.joinArrays(COMMAND_PSUBSCRIBE, patterns));
	}

	@Override
	public void punsubscribe(String... patterns) 
	{
		writer.writeArray(Utils.joinArrays(COMMAND_PUNSUBSCRIBE, patterns));
	}
	...
	private class SubcriptionThread extends Thread
	{
		SubcriptionThread()
		{
			setName("RedisSubscription-"+counter.getAndIncrement());
			setDaemon(true);
		}
		...
		public void run()
		{
			...
			while (isConnected() && (response = reader.readArray()) != null)
			{
				...
				if (response[0].equals("subscribe"))
					fireOnSubscribe(response[1], Utils.parseLong(response[2]));
				else if (response[0].equals("unsubscribe"))
					fireOnUnsubscribe(response[1], Utils.parseLong(response[2]));
				else if (response[0].equals("psubscribe"))
					fireOnPatternSubscribe(response[1], Utils.parseLong(response[2]));
				else if (response[0].equals("punsubscribe"))
					fireOnPatternUnsubscribe(response[1], Utils.parseLong(response[2]));
